...
int main(int argc, char** argv)
{
    try
    {
        ...
        {
            runSelfTests();
            std::cout << "All A-BIRCH and MBD-BIRCH self-tests passed\n";
            return 0;
        }
        ...
        if(argc == 13)
            return runPipeline(argv[1], argv[2], argv[3], argv[4],
                ...
                std::stod(argv[10]),static_cast<size_t>(std::stoul(argv[11])),
                std::stod(argv[12]));
        throw std::invalid_argument(
            "Usage: birch [features truth results report branching min_cluster "
            "pilot_points pilot_max_k threshold_override mbd_spread knn_k knn_fraction]");
    }
